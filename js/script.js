...
// Controller Class
class App {
  // I create the userCoords private field with the user's initial coordinates because I will need them to reposition the map on the initial coordinates if the user deletes all the workouts (Delete All)
  #userCoords = [];
  #map;
  #mapEvent;
  #workouts = [];
  // The markers array is used to save all the markers (which are leaflet objects) in a single array so that they can be retrieved (for example via the coordinates) when I have to modify a workout (and the corresponding marker) or delete a workout (and the corresponding marker)
  // It is also essential to integrate the Show All functionality which shows all the markers in the world
  #workoutMarkers = [];
  #mapZoomLevel = 13;
  ...
  _checkWorkouts() {
    // I disable the menu items if there are no workouts and restore the sort label (date)
    ...
    marker
      .bindPopup(
        L.popup({
          ...
          `You are <strong>here</strong>.<br> Please choose a place to add a workout`
        )
      )
      .openPopup();
  ...
  _handleClick(e) {
    // With each click, the first operation is to close all the open menus (sort and workout)
    this._closeAllMenus();

    // Now I have to filter the clicks in order to capture only clicks on the main menu links (show, delete and sort) and those on the workout-menu. So if I don't click on any of these links or if there is already an active edit-form or an active delete-confirmation I exit the entire function --> In fact when an edit-form or a delete-box is active I want the the user concentrates on them without being able to click elsewhere and create confusion (for example by opening other menus or showing the form to add the workout)
    // Guard Clause
    if (
    ...
    // I remove the active class from the main menu links. It will be added to the link I click on
    document
      .querySelectorAll('.menu__link.active')
      .forEach(link => link.classList.remove('active'));

    // Click to open the workout menu
    ...
    // Click on the main menu (show all / delete all / sort)
    if (e.target.closest('.menu__option')) {
      // If the links are disabled, exit the function
  ...
  _showEditForm(e) {
    ...
    // To ensure that I can position the form-edit exactly in the same top position of the workout list, I have to find the top position of the workout list with respect to the .workouts container, so I have to find the distance that the workout-list I clicked on has from the top of its container. This distance is equal to the distance the workout-list is from the top of the page (getboundingclientrect().top) minus (subtracted) the distance of the .workouts container from the top of the page. When we scroll the workouts container we must also add to this last subtraction the amount of scrolls we have performed on the .workouts container (scrollTop)
    formEdit.style.top = `${
